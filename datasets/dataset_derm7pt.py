...
def get_clinic_df(data_dir: str, num_classes: int):
    """Load the Derm7pt clinical photography train/validation split.
    ...
    Returns:
        tuple: ``(df_train, df_valid, mel_idx)``
    """
    df_train = _build_df(data_dir, 'train_set.csv', 'clinic', num_classes)
    df_valid = _build_df(data_dir, 'valid_set.csv', 'clinic', num_classes)
    return df_train, df_valid, 1


def get_clinic_test_df(data_dir: str, num_classes: int):
    """Load the Derm7pt clinical photography train/test split.
    ...
    """
    df_train = _build_df(data_dir, 'train_set.csv', 'clinic', num_classes)
    df_test  = _build_df(data_dir, 'test_set.csv',  'clinic', num_classes)
    return df_train, df_test, 1
