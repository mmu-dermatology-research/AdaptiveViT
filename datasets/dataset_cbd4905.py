...
def get_df_imbalanced_26k(data_dir: str):
    """Load the ~26 k imbalanced ISIC train/validation split.
    ...
    Returns:
        tuple: ``(df_train, df_valid, mel_idx)``
    """
    df_train = pd.read_csv(os.path.join(data_dir, 'isic_imbalanced_26k_train_set.csv'))
    df_train = df_train.rename(columns={'class': 'target'})

    df_valid = pd.read_csv(os.path.join(data_dir, 'isic_imbalanced_26k_valid_set.csv'))
    df_valid = df_valid.rename(columns={'class': 'target'})

    return df_train, df_valid, 1


def get_df_imbalanced_56k(data_dir: str):
    """Load the curated ~56 k imbalanced ISIC train/validation split.
    ...
    Returns:
        tuple: ``(df_train, df_valid, mel_idx)``
    """
    df_train = pd.read_csv(os.path.join(data_dir, 'curated-56k-train-complete-metadata.csv'))
    df_train = df_train.rename(columns={'class': 'target'})

    df_valid = pd.read_csv(os.path.join(data_dir, 'curated-56k-val-complete-metadata.csv'))
    df_valid = df_valid.rename(columns={'class': 'target'})
